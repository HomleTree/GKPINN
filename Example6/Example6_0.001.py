...
data = np.loadtxt("6—0.001.csv", delimiter=",", skiprows=1)  # 跳过标题行
...
class RBAGKPINN():
    ...
    def net_u(self, x, y):
        ...
        u = self.gknn1(a) + self.gknn2(a) * torch.exp((-y) / self.Diff) + self.gknn3(a) * torch.exp((-x) / self.Diff)
        return u
...
# 绘制3D曲面图
surf = ax.plot_surface(X, Y, E, cmap='jet', edgecolor='none')
