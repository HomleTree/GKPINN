...
#         self.fc1 = nn.Linear(1, 100)   # 第一层：输入层到第一个隐藏层
#         self.fc2 = nn.Linear(100, 100) # 第二层：第一个隐藏层到第二个隐藏层
#         self.fc3 = nn.Linear(100, 1)   # 第三层：第二个隐藏层到输出层
...
# # 创建一个大小为 (1000, 1) 的输入数据张量，1000 个样本，每个样本是一个长度为 1 的向量
# input_data = torch.randn(1000, 1)

# # 将输入数据传递给模型
# output = model(input_data)

# print(output.size())  # 输出结果的大小应该是 (1000, 1)，即每个样本对应一个预测输出

# print("第一层权重 W 的形状:", model.fc1.weight.shape)  # 输出 (100, 1)，表示有 100 个神经元，每个神经元有 1 个输入特征的权重
# print("第一层偏置 b 的形状:", model.fc1.bias.shape)   
...
# # 打印相关信息
# print("输入数据的形状:", input_data.shape)   # 输出 (1000, 1)
# print("第一层输出的形状:", output.shape)     # 输出 (1000, 100)，每个样本经过第一层后被扩展为 100 维
# print("第一层权重 W1 的形状:", W1.shape)  # 输出 (100, 1)，表示有 100 个神经元，每个神经元有 1 个输入特征的权重
# print("第一层偏置 b1 的形状:", b1.shape)    # 输出 (100,)，表示有 100 个神经元，每个神经元有一个独立的偏置项

# # 计算第一层的加权和 z1
# z1 = input_data.mm(W1.t()) + b1
# print("第一层加权和 z1 的形状:", z1.shape)    # 输出 (1000, 100)，与 self.fc1(input_data) 的输出相同
