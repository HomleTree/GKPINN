...
data = np.loadtxt("9—0.001.csv", delimiter=",", skiprows=0)  # 跳过标题行
...
class PINN():
    def __init__(self, u0, X0, X_f, X_lb, X_ub, epochs, Diff, X_star, u_star):
...
# model1 = PINN(u0, X0, X_f, X_lb, X_ub, epochs, Diff, X_star, u_star)
# model1.train()
# u_pred_pinn = model1.predict(X_star)

model = RBAGKPINN(u0, X0, X_f, X_lb, X_ub, epochs, Diff, X_star, u_star)
model.train()
...
# plt.subplot(1, 2, 1)
plt.plot(model.loss_0, '-.', label='loss_0')
...
# ax1.set_ylabel('t', labelpad=8, fontsize=16)
...
# 绘制3D曲面图
surf = ax.plot_surface(X, T, E, cmap='jet', edgecolor='none')
