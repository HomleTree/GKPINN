...
# # $ -0.01(u_{xx} + u_{yy}) + u_y = 0 $
# # $ u(x,0) = u(x,1) = 0, u(0,y) = 2sin(\pi y), u(1,y) = sin(\pi y) $
...
data = np.loadtxt("5—0.001.csv", delimiter=",", skiprows=1)  # 跳过标题行
...
class MNISTChebyKAN1(nn.Module):
    def __init__(self):
        super(MNISTChebyKAN1, self).__init__()
        self.chebykan_in = ChebyKANLayer(2, h_n, 5)
        self.chebykan = ChebyKANLayer(h_n, h_n, 5)
        self.chebykan_out = ChebyKANLayer(h_n, 1, 5)
        self.ln = nn.LayerNorm(h_n) # To avoid gradient vanishing caused by tanh

    def forward(self, x):
        # x = x.view(-1, nPt)  # Flatten the images
        x = self.chebykan_in(x)
        x = self.ln(x)
        x = self.chebykan(x)
        x = self.ln(x)
        # x = torch.tanh(x)
        x = self.chebykan_out(x)
        return x
...
class PINN():
    def __init__(self, X_f, X_lb, X_ub, X_rb, X_lftb, u_lb, u_ub, epochs, Diff, X_star, u_star):
        ...
        self.u_lb = torch.tensor(u_lb, dtype=torch.float32, requires_grad=True).to(device)
        ...
        self.u_ub = torch.tensor(u_ub, dtype=torch.float32, requires_grad=True).to(device)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            loss_u = torch.mean((u_lb_pred - self.u_ub) ** 2) + \
                     torch.mean((u_ub_pred - self.u_lb) ** 2) + \
                     torch.mean((u_rb_pred - 0) ** 2) + \
                     torch.mean((u_lftb_pred - 0) ** 2)
...
class RBAGKPINN():
    ...
    def net_u(self, x, y):
        ...
        u = self.gknn1(a) + self.gknn2(a) * torch.exp((-y) / self.Diff) 
        return u
...
# # 或者
# np.savetxt('u_pred_data1.csv', u_pred1, delimiter=',')  # 保存为CSV文件
# np.savetxt('u_pred_data2.csv', u_pred2, delimiter=',')  # 保存为CSV文件
...
# ax1 = fig.add_subplot(131, projection='3d')
fig2, ax2 = plt.subplots(figsize=(8, 6), subplot_kw={'projection': '3d'})
surf2 = ax2.plot_surface(X, Y, error_U, cmap='jet', edgecolor='none')
norm2 = mcolors.LogNorm(vmin=1e-4, vmax=1e-3)
cbar2 = plt.colorbar(surf2, ax=ax2, norm=norm2)
