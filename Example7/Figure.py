...
u_pred_pinn = np.loadtxt("u_pred_data1.csv", delimiter=",", skiprows=0) 
u_pred_pinn = u_pred_pinn.reshape(-1, 1)
u_pred_gkpinn = np.loadtxt("u_pred_data2.csv", delimiter=",", skiprows=0) 
...
bounds = [mean_gkpinn, 0.0038, 0.0065,mean_pinn]

fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')

# 绘制3D曲面图
surf = ax.plot_surface(X, T, Error_gkpinn, cmap='jet', edgecolor='none')
...
bounds = [0.25, 1.15, 2.0]

fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')

# 绘制3D曲面图
surf = ax.plot_surface(X, T, Error_pinn, cmap='jet', edgecolor='none')
